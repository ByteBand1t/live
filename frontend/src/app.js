...
function initSocket() {
  ...
  socket.on("vehicles:update", function(data) {
    ...
    document.getElementById("stats-update").textContent  =
      "Live-Update: " + new Date(data.timestamp).toLocaleTimeString("de-DE") + " · Realtime aktiv";
  });
}
...
function showLoadingPanel(title) {
  openPanel(
    ...
    '<div class="panel-loading"><div class="spinner"></div>Realtime-Daten…</div>'
  );
}
...
function handleBusClick(bus) {
  ...
  .then(function(course) {
    ...
    loadAndShowRoute(course, bus.color || "#E2001A");
  })
  .catch(function(err) {
...
function loadAndShowRoute(course, color) {
  var keys = (course.stopList || [])
...
function renderBusPanel(bus, course) {
  ...
  for (var i = 0; i < stopList.length; i++) {
    var stop = stopList[i];
    var t    = stop.time && stop.time.time ? stop.time.time : "";
...
function handleStationClick(station) {
  ...
  fetch(url)
    ...
    .catch(function(err) {
      console.error("[Departures] Fehler:", err);
      openPanel(
...
function renderStationPanel(station, departures) {
  ...
  openPanel(
    ...
    '<div class="departures-list">' +
      (html || '<div class="panel-loading"><div class="spinner"></div>Keine Abfahrten gefunden</div>') +
    '</div>'
  );
